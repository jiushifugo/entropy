...
class Config:
    ...
    daily_volume_limit_usd: float
    daily_risk_state_file: str
    # execution
    premium_persist_sec: float
    cooldown_sec: float
    settle_timeout_sec: float
    leg_slippage_bps: float
    entropy_leg_slippage_bps: float
    entropy_max_slippage_bps: float
    entropy_retry_once: bool
    entropy_retry_min_net_bps: float
...
def load_config(config_file: str = "config.yaml", env_file: str = ".env", *,
    ...
    entropy_leg_slip = float(_get(
        raw, "execution", "entropy_leg_slippage_bps", legacy_leg_slip))
    entropy_max_slip = float(_get(
        raw, "execution", "entropy_max_slippage_bps", entropy_leg_slip))
    hedge_leg_slip = float(_get(
        raw, "execution", "hedge_leg_slippage_bps", legacy_leg_slip))
    if min(entropy_leg_slip, entropy_max_slip, hedge_leg_slip) < 0:
        raise ConfigError("execution slippage values must be >= 0")
    if entropy_max_slip < entropy_leg_slip:
        raise ConfigError("execution.entropy_max_slippage_bps must be >= "
                          "entropy_leg_slippage_bps")
    daily_loss_action = str(_get(
        raw, "risk", "daily_loss_action", "halt")).strip().lower()
    daily_loss_action = daily_loss_action.replace("-", "_")
    if daily_loss_action not in ("halt", "profit_only"):
        raise ConfigError("risk.daily_loss_action must be 'halt' or "
                          "'profit_only'")
    profit_only_min_bps = float(_get(
        raw, "risk", "profit_only_min_bps", 1.0))
    if profit_only_min_bps < 0:
        raise ConfigError("risk.profit_only_min_bps must be >= 0")
    entropy_retry_min_net_bps = float(_get(
        raw, "execution", "entropy_retry_min_net_bps", 0.5))
    if entropy_retry_min_net_bps < 0:
        raise ConfigError("execution.entropy_retry_min_net_bps must be >= 0")
    entropy_resting_ttl_sec = float(_get(
        raw, "execution", "entropy_resting_ttl_sec", 0.0))
    ...
    return Config(
        ...
        upper_bps=upper,
        lower_bps=lower,
        exit_bps=float(_get(raw, "thresholds", "exit_bps", 0.5)),
        ...
        profit_only_min_bps=profit_only_min_bps,
        daily_volume_limit_usd=float(_get(
            raw, "risk", "daily_volume_limit_usd", 0.0)),
        daily_risk_state_file=str(_get(
            raw, "risk", "state_file", "logs/daily-risk.json")),
        ...
        leg_slippage_bps=legacy_leg_slip,
        entropy_leg_slippage_bps=entropy_leg_slip,
        entropy_max_slippage_bps=entropy_max_slip,
        entropy_retry_once=bool(_get(
            raw, "execution", "entropy_retry_once", False)),
        entropy_retry_min_net_bps=entropy_retry_min_net_bps,
        entropy_resting_ttl_sec=entropy_resting_ttl_sec,
